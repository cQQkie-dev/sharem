...
class CustomWinAPIs():
    ...
    def VirtualAllocEx(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        ...
        # TODO: WILLIAM FIX THIS LATER

        retVal = 0
        try:
            # TODO: round down the memory
    ...
    @HookAPI
    def VirtualQueryEx(hProcess: 'HANDLE', lpAddress: 'LPCVOID', lpBuffer: 'PMEMORY_BASIC_INFORMATION', dwLength: 'SIZE_T', uc: Uc, em: EMU) -> 'SIZE_T':
        # Author: WILLIAM LOCHTE
        # 'VirtualQueryEx': (4, ['HANDLE', 'LPCVOID', 'PMEMORY_BASIC_INFORMATION', 'SIZE_T'], ['hProcess', 'lpAddress', 'lpBuffer', 'dwLength'], 'SIZE_T')

        page_size = 4096 # TODO: Use this value to align addresses to pages
        next_page = ((lpAddress // page_size) + 1) * page_size

        mem_info = get_MEMORY_BASIC_INFORMATION(uc, lpBuffer, em)
        mem_info.BaseAddress = next_page     # lpAddress
        mem_info.AllocationBase = lpAddress  # lpAddress     TODO: align to page start
        mem_info.AllocationProtect = 0x40    # PAGE_EXECUTE_READWRITE
        mem_info.PartitionId = 2             # TODO: UPDATE THIS TO BE A REAL VALUE. Not sure what it does
        mem_info.RegionSize = page_size      
        mem_info.State = 0x1000              # MEM_COMMIT
        mem_info.Protect = 0x40              # PAGE_EXECUTE_READWRITE
        mem_info.Type = 0x40000              # MEM_MAPPED

        mem_info.writeToMemory(uc, lpBuffer) # Write to the memory

        # Not sure I should display this since it is all artificial...
        lpBuffer = makeStructVals(uc, mem_info, lpBuffer) # Display the output
    
        retVal = len(bytes(mem_info))        # Length of the MEMORY_BASIC_INFORMATION block
        uc.reg_write(UC_X86_REG_EAX, retVal)

        retValStr = hex(retVal)
    
        # retValStr, pVals, to_str_skip
        return retValStr, [hProcess, lpAddress, lpBuffer, dwLength], ["lpBuffer"]
    ...
    def GetMessageA(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        pTypes= ['LPMSG', 'HWND', 'UINT', 'UINT']
        pNames= ['lpMsg', 'hWnd', 'wMsgFilterMin', 'wMsgFilterMax']
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        ...
        logged_calls= ("GetMessageA", hex(callAddr), (retValStr), 'BOOL', pVals, pTypes, pNames, False)
        return logged_calls, stackCleanup(uc, em, esp, len(pTypes))

    def xxExpandEnvironemtnStringsA(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        pTypes= ['LPCSTR', 'LPSTR', 'DWORD']
        pNames= ['lpSrc', 'lpDst', 'nSize']
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        ...
        logged_calls= ("ExpandEnvironemtnStringsA", hex(callAddr), (retValStr), 'DWORD', pVals, pTypes, pNames, False)
        return logged_calls, stackCleanup(uc, em, esp, len(pTypes))
 ...
 'PF_COMPARE_EXCHANGE_DOUBLE', 14: 'PF_COMPARE_EXCHANGE128', 15: 'PF_COMPARE64_EXCHANGE128', 23: 'PF_FASTFAIL_AVAILABLE', 1: 'PF_FLOATING_POINT_EMULATED', 0: 'PF_FLOATING_POINT_PRECISION_ERRATA', 3: 'PF_MMX_INSTRUCTIONS_AVAILABLE', 12: 'PF_NX_ENABLED', 9: 'PF_PAE_ENABLED', 8: 'PF_RDTSC_INSTRUCTION_AVAILABLE', 22: 'PF_RDWRFSGSBASE_AVAILABLE', 20: 'PF_SECOND_LEVEL_ADDRESS_TRANSLATION', 13: 'PF_SSE3_INSTRUCTIONS_AVAILABLE', 21: 'PF_VIRT_FIRMWARE_ENABLED', 6: 'PF_XMMI_INSTRUCTIONS_AVAILABLE', 10: 'PF_XMMI64_INSTRUCTIONS_AVAILABLE', 17: 'PF_XSAVE_ENABLED', 29: 'PF_ARM_V8_INSTRUCTIONS_AVAILABLE', 30: 'PF_ARM_V8_CRYPTO_INSTRUCTIONS_AVAILABLE', 31: 'PF_ARM_V8_CRC32_INSTRUCTIONS_AVAILABLE', 34: 'PF_ARM_V81_ATOMIC_INSTRUCTIONS_AVAILABLE'}
    ...
    def xxTlsSetValue(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        pTypes= ['DWORD', 'LPVOID']
    ...
    def xxNetShareGetInfo(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        ...
        if pVals[2] == 0:
            shareinfo = get_SHARE_INFO_0(uc, pVals[3], em)
            shareinfo.shi0_netname = 0x88888888
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 1:
            shareinfo = get_SHARE_INFO_1(uc, pVals[3], em)
            shareinfo.shi1_netname = 0x88888888
            shareinfo.shi1_type = 0x00000001
            shareinfo.shi1_remark = 0x2
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 2:
            shareinfo = get_SHARE_INFO_2(uc, pVals[3], em)
            shareinfo.shi2_netname = 0x88888888
            shareinfo.shi2_type = 0x00000001
            shareinfo.shi2_remark = 0x2
            shareinfo.shi2_permissions = 0x3
            shareinfo.shi2_max_uses = 0x4
            shareinfo.shi2_current_uses = 0x5
            shareinfo.shi2_path = 0x6
            shareinfo.shi2_passwd = 0x7
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 501:
            shareinfo = get_SHARE_INFO_501(uc, pVals[3], em)
            shareinfo.shi501_netname = 0x88888888
            shareinfo.shi501_type = 0x00000001
            shareinfo.shi501_remark = 0x2
            shareinfo.shi501_flags = 0x3
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 502:
            shareinfo = get_SHARE_INFO_502(uc, pVals[3], em)
            shareinfo.shi502_netname = 0x88888888
            shareinfo.shi502_type = 0x00000001
            shareinfo.shi502_remark = 0x5
            shareinfo.shi502_permissions = 0x00000001
            shareinfo.shi502_max_uses = 0x7
            shareinfo.shi502_current_uses = 0x8
            shareinfo.shi502_path = 0x9
            shareinfo.shi502_passwd = 0x11
            shareinfo.shi502_reserved = 0x0
            shareinfo.shi502_security_descriptor = 0x13
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 503:
            shareinfo = get_SHARE_INFO_503(uc, pVals[3], em)
            shareinfo.shi0_netname = 0x88888888
            shareinfo.shi503_type = 0x00000001
            shareinfo.shi503_remark = 0x2
            shareinfo.shi503_permissions = 0x00000001
            shareinfo.shi503_max_uses = 0x4
            shareinfo.shi503_current_uses = 0x5
            shareinfo.shi503_path = 0x6
            shareinfo.shi503_passwd = 0x7
            shareinfo.shi503_reserved = 0x0
            shareinfo.shi503_security_descriptor = 0x9
            shareinfo.writeToMemory(uc, pVals[3])
            uc.mem_write(pVals[3])
        elif pVals[2] == 1005:
            shareinfo = get_SHARE_INFO_1005(uc, pVals[3], em)
    ...
    def CheckTokenMembership2(self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU):
        ...
        logged_calls= ("CheckTokenMembership2", hex(callAddr), (retValStr), 'BOOL', pVals, pTypes, pNames, False)
        return logged_calls, stackCleanup(uc, em, esp, len(pTypes))
